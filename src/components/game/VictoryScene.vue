...
onMounted(() => {
  ...
  const hits = props.winner === 'jannabi' ? [960, 1280, 1600, 1920, 2240] : [1280];
...
<template>
  <dialog ref="dialog" class="victory-scene" :aria-label="`${characterName(winner)} 승리 연출`" @cancel.prevent="finish">
    <div class="victory-stage" :class="{ lariat: winner === 'jannabi' }">
      <p class="victory-title">{{ winner === 'jannabi' ? '다섯 바퀴! 멈출 수 없는 잔나비!' : '마지막 한 음까지, 완벽하게!' }}</p>
      <div class="stage" role="img" :aria-label="winner === 'jannabi' ? '잔나비가 양팔을 옆으로 뻗고 다섯 바퀴 회전하며 베짱이를 연속으로 때리고 마지막 타격에 날려 보냅니다.' : '베짱이가 바이올린을 휘둘러 잔나비를 빙글빙글 날려 보내고 인사합니다.'">
        <div class="floor" />
      ...
      <strong class="victory-caption">{{ winner === 'jannabi' ? '잔나비의 더블 래리어트!' : '베짱이의 화려한 피날레!' }}</strong>
...
.champion { left: 12%; z-index: 1; transform-origin: 50% 90%; animation: victory-bow 3.2s both; }
...
@keyframes victory-bow { 0%, 12% { transform: none; } 32% { transform: translateX(-6%) rotate(-12deg); } 42%, 50% { transform: translateX(44%) rotate(14deg); } 65% { transform: translateX(10%); } 80% { transform: translateX(10%) rotate(18deg); } 100% { transform: translateX(10%); } }
@keyframes fly-away { 0%, 39% { transform: none; } 43% { transform: translate(8%, -3%) rotate(20deg) scaleY(.8); } 64% { transform: translate(95%, -95%) rotate(230deg) scale(.65); opacity: 1; } 80%, 100% { transform: translate(230%, -180%) rotate(540deg) scale(.2); opacity: 0; } }
@keyframes hit-flash { 0%, 37%, 56%, 100% { opacity: 0; } 41%, 47% { opacity: 1; } }
...
@keyframes camera-shake { 0%, 39%, 49%, 100% { transform: none; } 41%, 45% { transform: translateX(-4px); } 43%, 47% { transform: translateX(4px); } }
