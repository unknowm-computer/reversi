...
interface Props { board: Cell[]; legal: number[]; turn: Color; interactive: boolean; lastMove: number | null; flipped: number[]; revision: number }
...
function label(index: number): string {
  return `${'ABCDEFGH'[index % 8]}${Math.floor(index / 8) + 1}, ${props.board[index] === 'black' ? '흑돌' : props.board[index] === 'white' ? '백돌' : '빈칸'}${moves.value.has(index) ? ', 착수 가능' : ''}${props.lastMove === index ? ', 마지막 착수' : ''}`;
}
