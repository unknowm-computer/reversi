...
export function useMoveImpact(state: () => GameState, onHit: () => void) {
  const scene = ref<{ actor: Color; kind: 'capture'; count: number } | null>(null);
  const preview = ref<GameState | null>(null);
  const busy = ref(false);
  let hitTimer: number | undefined, endTimer: number | undefined;
  function cancel(): void {
    window.clearTimeout(hitTimer); window.clearTimeout(endTimer);
  ...
  watch(state, (next, before) => {
    ...
    hitTimer = window.setTimeout(() => { onHit(); scene.value = null; preview.value = null; }, IMPACT_MS);
